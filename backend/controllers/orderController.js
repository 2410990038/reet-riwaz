...
// In-memory store for idempotency keys (cleared every hour)
const processedKeys = new Map();
setInterval(() => processedKeys.clear(), 60 * 60 * 1000);

// @route  POST /api/orders/place
const placeOrder = async (req, res) => {
  const { clerkUserId, userEmail, items, shippingAddress, paymentMethod, totalAmount, idempotencyKey } = req.body;

  try {
    if (!items || items.length === 0) return res.status(400).json({ message: 'No items in order' });

    // ✅ Idempotency check — if same key already processed, return existing order
    if (idempotencyKey) {
    ...
    if (idempotencyKey) {
      processedKeys.set(idempotencyKey, order._id.toString());
    }
