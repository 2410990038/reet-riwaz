...
//       const usersMap = {};
//       (Array.isArray(ordersData) ? ordersData : []).forEach(o => {
//         // ✅ FIX: Give each guest their own unique key based on email or order id
//         const userId = o.clerkUserId === 'guest'
//           ? `guest_${o.userEmail || o._id}`
//           : o.clerkUserId;
...
//             totalSpent: 0,
//             lastOrder: o.createdAt,
//             isGuest: o.clerkUserId === 'guest',
//           };
//         }
//         usersMap[userId].orders += 1;
//         usersMap[userId].totalSpent += o.totalAmount || 0;

//         // ✅ Always update name, city, phone from the latest order
//         if (new Date(o.createdAt) > new Date(usersMap[userId].lastOrder)) {
//           usersMap[userId].lastOrder = o.createdAt;
//           usersMap[userId].name = `${o.shippingAddress?.firstName || ""} ${o.shippingAddress?.lastName || ""}`.trim();
//           usersMap[userId].phone = o.shippingAddress?.phone || "—";
//           usersMap[userId].city = o.shippingAddress?.city || "—";
//         }
//       });
//       setUsers(Object.values(usersMap).sort((a, b) => b.totalSpent - a.totalSpent));
...
//                                 {u.name || "—"}
//                                 {u.isGuest && <span className="ml-2 text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">Guest</span>}
//                               </td>
//                               <td className="px-4 py-3 text-gray-600">{u.email}</td>
//                               <td className="px-4 py-3 text-gray-600">{u.phone}</td>
//                               <td className="px-4 py-3 text-gray-600">{u.city}</td>
...
export default function AdminPanel() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const usersMap = {};
      (Array.isArray(ordersData) ? ordersData : []).forEach((o) => {
        const userId = o.clerkUserId === "guest" ? `guest_${o.userEmail || o._id}` : o.clerkUserId;
        if (!usersMap[userId]) {
          usersMap[userId] = {
            id: userId,
            email: o.userEmail,
  ...
  // ✅ NEW: Open edit panel for a product
  const startEditProduct = (p) => {
    ...
    setEditImageInput(p.image || "");
  };

  // ✅ NEW: Save edited product — calls PUT /api/admin/products/:id
  const updateProduct = async (productId) => {
    const updated = { ...editForm, image: editImageInput };
    await fetch(`${BASE_URL}/admin/products/${productId}`, {
      method: "PUT",
      headers,
      body: JSON.stringify({
        ...updated,
        priceValue: Number(updated.priceValue),
              ...
              <div>
                ...
                <div className="bg-white rounded-lg shadow p-6 mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button onClick={addProduct} className="bg-green-600 text-white rounded px-6 py-2 hover:bg-green-700 font-semibold col-span-1 sm:col-span-2">
                      Add Product
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {products.map((p) => (
                    <div key={p._id} className="bg-white rounded-lg shadow p-4">
                      ...
                      {/* Product Image — shows placeholder if broken */}
                      <div className="relative w-full h-40 mb-3 bg-gray-100 rounded overflow-hidden">
                        ...
                        <div
                          style={{ display: "none" }}
                          className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 text-sm gap-1"
                        >
                      ...
                      <div className="font-bold">{p.name}</div>
                      <div className="text-green-600 font-semibold">{p.price}</div>
                      <div className="text-sm text-gray-500 capitalize mb-3">{p.category}</div>

                      {/* ✅ EDIT PANEL — shows when this product is being edited */}
                      {editingProduct === p._id ? (
                        <div className="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                          <p className="text-xs font-semibold text-orange-700 mb-2">✏️ Edit Product</p>
                          ...
                          <div className="space-y-2 mb-3">
                            <input
                              ...
                              className="w-full border rounded px-2 py-1.5 text-sm focus:outline-none focus:border-orange-400"
                            />
                            <div className="flex gap-2">
                              <input
                                ...
                                className="w-1/2 border rounded px-2 py-1.5 text-sm focus:outline-none focus:border-orange-400"
                              />
                              <input
                                placeholder="Price Value"
                                type="number"
                                value={editForm.priceValue}
                                onChange={(e) => setEditForm({ ...editForm, priceValue: e.target.value })}
                                className="w-1/2 border rounded px-2 py-1.5 text-sm focus:outline-none focus:border-orange-400"
                              />
                            </div>
                            <div className="flex gap-2">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 bg-orange-500 text-white py-1.5 rounded text-sm font-semibold hover:bg-orange-600"
                            >
                              💾 Save
                            </button>
                            <button
                        ...
                        <button
                          ...
                          className="w-full bg-orange-100 text-orange-700 py-1.5 rounded hover:bg-orange-200 text-sm font-semibold mb-2"
                        >
                          ✏️ Edit Product
                        </button>
                      )}
