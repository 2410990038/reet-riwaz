...
export default function Payment() {
  ...
  // Generate a unique key for this payment session — created once per page load
  ...
  const handlePayNow = async () => {
    if (hasFired.current || done || processing) return;
    hasFired.current = true;
    setProcessing(true);
    setDone(true);
    setError("");
    ...
    try {
      ...
      const orderData = {
        ...
        paymentMethod: method,
        totalAmount: amountToPay,
        idempotencyKey: idempotencyKey.current  // ✅ unique key per session
      };
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <button
              disabled={done}
              onClick={handlePayNow}
              className={`w-full mt-4 ${done ? "bg-green-400 cursor-not-allowed opacity-70 pointer-events-none" : "bg-green-600 hover:bg-green-700"} text-white py-3 rounded-lg font-bold transition`}
            >
              {processing ? "Processing... Please wait ⏳" : `Pay ₹${amountToPay.toLocaleString()}`}
            </button>
